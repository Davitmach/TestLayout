...
`,
    ...
    },
    {
      ...
      img: "/case/GoodFood/1.jpg",
      title: "GoodFood - полезное питание",
      date: "Февраль 2025",
      page: {
        inner:'/case/GoodFood/1.jpg',
        img: ["/case/GoodFood/1.jpg", "/case/GoodFood/2.png", "/case/GoodFood/3.png"],
        info: [
          {
            title: `Как к нам обратились за разработкой мобильного приложения.
Задача стояла четкая`,
            description: `— создать сервис, который поможет пользователям не просто заказывать продукты, а делать это с умом, заботой о здоровье и с максимальным удобством. Проект оказался интересным: мы создавали цифрового помощника для тех, кто заботится о своем питании.
`,
          },
          {
            title: `Исходная задача`,
            description: `Клиент хотел приложение, в котором можно:

            Подобрать продукты под индивидуальные цели — от рациона на каждый день до специализированных диет.

            Быстро и просто заказывать свежие продукты с доставкой в течение часа.

            Получать только проверенные по качеству товары.

            Участвовать в бонусной программе.

            Нашей целью было разработать не просто маркетплейс, а визуально приятный и понятный инструмент для ежедневного использования.
`,
          },
          {
            title: "Процесс разработки",
            description: `Мы начали с глубокой проработки пользовательского пути. Анализировали, как человек выбирает продукты, что для него важно на разных этапах: фильтры, сортировка, карточки товаров, описание, оформление заказа. На основании этих данных разработали удобную и логичную структуру интерфейса.

`,
          },
          {
            title: "Дизайн-система",
            description: `Визуальный стиль приложения построен на контрасте: глубокий черный фон и акценты ярко-салатового цвета (#E8FE74), который ассоциируется со свежестью, экологичностью и пользой.
`,
          },
          {
            title: "Создали единую дизайн-систему:",
            description: `Иконки: минималистичные и читаемые, в едином ключе.

            Компоненты: кнопки, таб-бары, карточки — всё функционально и единообразно.

            Иллюстрации: дружелюбные изображения фруктов и овощей добавляют тепла и жизни в интерфейс.

            Карточки товаров снабжены фильтрами, рекомендательной системой, возможностью быстро изменить количество товара.

            Особое внимание уделили App Store-превью — каждый экран не только визуально отражает суть приложения, но и сразу доносит ценность: проверка качества, скорость доставки, бонусы, подбор под рацион.
...
`,
          ...
          {
            title: "Результат",
            description: `Мы запустили мобильное приложение, которое:

Помогает пользователям быстро находить нужные продукты по фильтрам.

         Формирует полезный рацион с учётом витаминов и нужд организма.

           Обеспечивает быструю доставку, сохраняя свежесть продуктов.

          
Строит лояльность через геймификацию и бонусную программу.

Приложение уже получило высокую оценку пользователей за понятный интерфейс, приятный дизайн и реальную пользу в повседневной жизни.
          ...
          return (
            <div key={index} onClick={() => handleCaseClick(e)}>
              <div className="Img_box">
                  ...
                  <img height={444} src={e.img} alt={e.title} />
