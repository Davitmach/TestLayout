...
const Dizayn = ()=> {
  return(
    <>
    <div><h3>Почему первое впечатление решает все</h3 ></div>
    <div className="Steps">
      <div>
        <div><span>01</span></div>
        <div>
          <div><h1>Мы не используем шаблоны и не гонимся за трендами ради трендов</h1></div>
      ...
      <div>
        <div><span>02</span></div>
        <div>
          <div><h1>Человек делает выводы о вашем бизнесе за считанные секунды</h1></div>
      ...
      <div>
        <div><span>03</span></div>
        <div>
          <div><h1>Хороший дизайн — это не только про красоту, но и про удобство</h1></div>
      ...
      <div>
        <div><span>04</span></div>
        <div>
          <div><h1>Каждая кнопка, каждая анимация, каждая тень — всё имеет значение</h1></div>
...
const Managment = ()=> {
  return(
    <>
    <div><h3>Менеджмент продуктов и проектов: когда важен не только результат, но и процесс</h3></div>
...
<div className="Info">
  <div>
    ...
    <div><p>Это профессиональная студия, специализирующаяся на быстром создании современных, функциональных и эффективных решений.  Мы не работаем шаблонно. Каждое решение разрабатывается с нуля, под конкретные задачи. Мы погружаемся в бизнес, понимаем его аудиторию и только после этого создаём продукт.</p></div>
  </div>
